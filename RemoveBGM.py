...
from rbgmLib import dataset
from rbgmLib import nets
from rbgmLib import spec_utils
from rbgmLib import utils
...
def RemoveBGM(path,out_dir):
    p = argparse.ArgumentParser()
    p.add_argument('--gpu', '-g', type=int, default=-1)
    p.add_argument('--pretrained_model', '-P', type=str, default='models/removebgm.pth')
    p.add_argument('--input', '-i', default=path)
    ...
    p.add_argument('--output_dir', '-o', type=str, default=out_dir)
    args = p.parse_args()
    ...
    # print('done')
    out_path='{}{}.wav'.format(output_dir, basename)
...
if __name__ == '__main__':
    RemoveBGM("wavs/y0AiSZFhOvA.wav","wavs-vr/")
